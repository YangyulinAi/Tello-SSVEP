...
class SSVEPStimulus:
    ...
    def open_stimulus_window(self,mode):
        frequencies = [float(freq) for freq in frequency_entry.get().split(',') if freq]
        duration = float(duration_entry.get())
        num_stimuli = int(num_stimuli_entry.get())
...
tk.Label(root, text="Enter Number of Stimuli (1-4):").pack()
num_stimuli_entry = tk.Entry(root)
num_stimuli_entry.pack()
...
tk.Label(root, text="Enter Frequencies (Hz, comma-separated, optional):").pack()
...
mode_combobox = ttk.Combobox(root, textvariable=mode_var, values=["train", "test"])
mode_combobox.pack()
mode_combobox.set("train")  # Default set to train

# Start button
start_button = tk.Button(root, text="Start Stimulus", command=lambda: open_stimulus_window(mode_var.get()))
start_button.pack()
