    ...
    def __init__(self, fig, channels_to_plot=[0, 1, 2], buffer_samples=500,
        ...
        for i, (ch, ch_name) in enumerate(zip(channels_to_plot, channel_names)):
            ...
            self.dsp_axs.append(plt.subplot2grid(shape=(self.n_channels, ncols*2),
                                                 loc=(ch, 1), colspan=1))
            # if i > 0:
            #     self.raw_axs[-1].sharex(self.raw_axs[-2])
            #     self.dsp_axs[-1].sharex(self.dsp_axs[-2])
...
def real_time_stream(stream_name):
    ...
    # set interactive
    # TODO:
        # Move to an animator function and specify number of channels to plot
        # Move classifier barplot to animator and take only the class_output
        # from buffer
    while timer.sec() <= demo_time:
        ...
        if dt >= buff_refresh:
            ...
            # for i in range(7):
            #     ax0.plot(buffer.timestamps, buffer.raw_data[:, i])
            #     ax1.plot(buffer.timestamps, buffer.data[:, i])
            # # plot the model probability output
            # cls_ax.bar(CLASS_LBLS, np.mean(buffer.class_data[-8:, -1]))

            # cls_ax.set_yticks([0,1])
            # cls_ax.set_xticks(np.arange(n_classes))
            ...
            # cls_ax.set_ylabel('Probabilities')
            # cls_ax.set_xlabel('Class Labels')
